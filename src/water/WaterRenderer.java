...
public class WaterRenderer {
	...
	public WaterRenderer(ModelLoader loader, WaterShader shader, Matrix4f projectionMatrix, WaterFrameBuffer fbos) {
		this.shader = shader;
		this.fbos = fbos;
		this.dudvTexture = loader.loadTexture(DUDV_MAP);
		this.normalMapTexture = loader.loadTexture(NORMAL_MAP);
		shader.start();
	...
	public void render(List<WaterTile> water, Camera camera, Light sun) {
		...
		for (WaterTile tile : water) {
			Matrix4f modelMatrix = Maths.getTransformMatrix(
					new Vector3f(tile.getX(), tile.getHeight(), tile.getZ()), new Vector3f(0, 0, 0),
					WaterTile.TILE_SIZE);
			shader.loadModelMatrix(modelMatrix);
			GL11.glDrawArrays(GL11.GL_TRIANGLES, 0, quad.getNumVertex());
		}
		unbind();
	}
